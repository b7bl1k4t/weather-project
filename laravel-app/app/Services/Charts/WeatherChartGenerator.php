...
class WeatherChartGenerator
{
    ...
    public function __construct(
        ...
        private readonly WeatherFixtureSeeder $seeder
    ) {
        $this->chartDir = $this->ensureChartDir();
        $this->fontPath = base_path('vendor/amenadiel/jpgraph/src/fonts/DejaVuSans.ttf');
    }

    public function ensureData(int $minimum = 50): void
    {
        $pdo = $this->db->getPdo();
        $this->seeder->seedIfBelow($pdo, $minimum);
        $this->seeder->seedWindowIfBelow($pdo, '-24 hours', 'now', 24, 36);
        $this->seeder->seedWindowIfBelow($pdo, '-7 days', 'now', 28, 42);
        $this->seeder->seedWindowIfBelow($pdo, '-30 days', 'now', 40, 60);
    }
    ...
    private function hourlyTemperatureSeries(): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($bucket !== '') {
                $map[(new DateTimeImmutable($bucket))->format('Y-m-d H:00:00')] = (float) ($row->avg_temp ?? 0);
            }
        }
    ...
    private function dailyAveragesSeries(): array
    {
        ...
        foreach ($rows as $row) {
            $bucket = (string) ($row->bucket ?? '');
            if ($bucket !== '') {
                $map[(new DateTimeImmutable($bucket))->format('Y-m-d')] = (float) ($row->avg_temp ?? 0);
            }
        }
